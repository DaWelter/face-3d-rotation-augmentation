...
def assert_similar_images(reference : Path, output : Path):
    ...
    for name in reference_images:
        ref = np.asarray(Image.open(reference / name)).astype(np.int32)
        img = np.asarray(Image.open(output / name)).astype(np.int32)
        num_pixels_mid_deviation = np.count_nonzero(np.abs(ref - img) > 5)
        num_pixels_high_deviation = np.count_nonzero(np.abs(ref - img) > 50)
        # 100% binary compatibility cannot be achieved across different systems.
        # If in doubt, compare images visually.
        # test/data/300wlp_reference_output
        # vs
        # /tmp/pytest-of-<username>/pytest-current/test_300wlp_reproductioncurrent/output/
        assert num_pixels_mid_deviation < 40, f"Mismatch in image {name}"
        assert num_pixels_high_deviation < 4, f"Mismatch in image {name}"
...
def assert_similar_labels(reference : Path, output : Path):
    with h5py.File(str(reference)+'.h5','r') as refh5, h5py.File(str(output)+'.h5', 'r') as outh5:
        ...
        assert_sequence_starts_ok(outh5['sequence_starts'][...], len(refh5['images'])) # type: ignore[arg-type]
        for k, ref in refh5.items():
            out = outh5[k]
            assert isinstance(out, h5py.Dataset)
            assert ref.dtype == out.dtype
            if k == 'quats':
                # Quaternions represent the same rotation if they are negated.
                a, b = out[...], ref[...]
                if np.allclose(a[:1], -b[:1], atol=1.e-3):
                    b = -b
                np.testing.assert_allclose(a, b, err_msg=f"Mismatch at dataset {k}")
            elif ref.dtype in (np.float32, np.float64):
...
def test_300wlp_reproduction(tmpdir):
    tmpdir = Path(tmpdir)
    main(str(Path(__file__).parent/'data/300wlp_miniset.zip'), str(tmpdir/'output.h5'), 1<<32, enable_vis=False, angle_step=5., prob_closed_eyes=0, prob_spotlight=0)
